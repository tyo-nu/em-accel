...
n_Rref = sum(n_rxn_steps(1:n_rev_rxns));
...
while n_Kset < nKsets                                                       % while loop needed when screening for locally stable parameter sets to continue sampling until nKsets stables sets are sampled
    ...
    if check_local_stability                                                
        stable_or_nah = is_Kset_locally_stable(Network_Data, K, fractions); % check if parameter set is locally stable     
...
fprintf(1,'\n');
fprintf(1,'%d models generated!', nKsets);
fprintf(1,'\n');
fprintf(1,'\n');
